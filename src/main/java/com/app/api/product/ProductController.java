...
public class ProductController {
    ...
    @ApiOperation(value = "List of products by id", response = ProductResponse.class)
    @RequestMapping(value = "/products/{warehouseId}", method = RequestMethod.GET)
    public ProductResponse getProductsByPageByUser(
        @ApiParam(value = ""    )               @RequestParam(value = "page"  ,  defaultValue="0"   ,  required = false) Integer page,
        @ApiParam(value = "between 1 to 1000" ) @RequestParam(value = "size"  ,  defaultValue="20"  ,  required = false) Integer size,
        @RequestParam(value = "productid"  , required = false) Integer productId,
        @RequestParam(value = "category"   , required = false) String  category,
       ...
       // Page<Product> productPage = productRepo.findAll(org.springframework.data.domain.Example.of(qry), pageable);
        Page<Product> productPage = productRepo.findByWarehouseId(warehouseId, pageable);
        resp.setPageStats(productPage, true);
        resp.setItems(productPage.getContent());
        return resp;
    }

    @ApiOperation(value = "List of products by not id", response = ProductResponse.class)
    @RequestMapping(value = "/product/{warehouseId}", method = RequestMethod.GET)
    public ProductResponse getProductsByPageByOtherUsers(
        @ApiParam(value = ""    )               @RequestParam(value = "page"  ,  defaultValue="0"   ,  required = false) Integer page,
        @ApiParam(value = "between 1 to 1000" ) @RequestParam(value = "size"  ,  defaultValue="20"  ,  required = false) Integer size,
        @RequestParam(value = "productid"  , required = false) Integer productId,
        @RequestParam(value = "category"   , required = false) String  category,
        ...
        // Page<Product> productPage = productRepo.findAll(org.springframework.data.domain.Example.of(qry), pageable);
        Page<Product> productPage = productRepo.findAllButId(warehouseId, pageable);
        resp.setPageStats(productPage, true);
        resp.setItems(productPage.getContent());
        return resp;
    }
